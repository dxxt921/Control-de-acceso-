package com.iotaccess.presentation.controllers;
...
public class ConfigController {
    ...
    /**
     * API: Obtiene la lista de sesiones recientes.
     */
    @GetMapping("/api/session/recent")
    @ResponseBody
    public ResponseEntity<List<String>> getRecentSessions() {
        return ResponseEntity.ok(accessService.getRecentSessions());
    }

    /**
     * API: Obtiene el historial de renombramientos de sesiones.
     */
    @GetMapping("/api/session/rename-history")
    @ResponseBody
    public ResponseEntity<List<Map<String, String>>> getRenameHistory() {
        return ResponseEntity.ok(accessService.getRenameHistory());
    }

    /**
     * API: Reconecta el Arduino a la sesión CSV existente o a una específica.
    ...
    public ResponseEntity<Map<String, Object>> reconnectSession(
            @RequestParam String portName,
            @RequestParam(required = false) String sessionFile) {

        Map<String, Object> response = new HashMap<>();

        try {
            log.info("Solicitud de reconexión: puerto={}, archivo={}", portName, sessionFile);
            ...
            accessService.reconnectSession(portName, sessionFile);
